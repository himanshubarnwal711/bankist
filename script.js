...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    labelWelcome.textContent = `Welcome back,n${
...
movements.forEach(function (movement, index) {
  if (movement > 0) {
    console.log(`Movement of ${index + 1}: You deposited ${movement}.`);
  } else {
    console.log(
      `Movement of ${index + 1}: You withdrew ${Math.abs(movement)}.`
    );
  }
});
...
let dogs = [...dogsJulia.slice(1, 3), ...dogsKate];
console.log(dogs);
...
const calcAverageHumanAge = function (dogsAge) {
  let humanAge = dogsAge
    .map(age => (age >= 2 ? 2 * age : 16 + age * 4))
    .filter(age => age >= 18);

  const average = humanAge.reduce((acc, age) => acc + age, 0) / humanAge.length;
  return `${humanAge}\n${average}`;
};
